 ...
 *
 * <h2>SelectorSoup syntax</h2>
 * <p>
 * A selector is a chain of simple selectors, separated by combinators. SelectorSoups are <b>case insensitive</b> (including against
...
open class SelectorSoup {
    ...
    public static func select(_ query: String, _ root: Element)throws->Elements {
        return try SelectorSoup(query, root).select()
    }
    ...
    public static func select(_ evaluator: Evaluator, _ root: Element)throws->Elements {
        return try SelectorSoup(evaluator, root).select()
    }
