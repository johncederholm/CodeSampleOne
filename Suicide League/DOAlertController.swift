...
class DOAlertController : UIViewController, UITextFieldDelegate, UIViewControllerTransitioningDelegate {
    ...
    // BlurView
    fileprivate var blurView = UIVisualEffectView(effect: UIBlurEffect(style: .light))
    ...
    var alertViewBgColor = UIColor.blue
    ...
    var titleFont = UIFont(name: textFontBold, size: 16)
    var titleTextColor = UIColor.black
    
    // MessageView
    fileprivate var messageView = UILabel()
    var messageFont = UIFont(name: textFont, size: 13)
    ...
    // TextViews
    fileprivate(set) var textViews: [AnyObject]?
    fileprivate let textViewHeight: CGFloat = 90.0
    var textViewBgColor = UIColor.white
    fileprivate let textViewCornerRadius: CGFloat = 4.0
    ...
    var buttonFont: [DOAlertActionStyle : UIFont?] = [
        .default : UIFont(name: buttonBold, size: 16),
        .cancel  : UIFont(name: buttonBold, size: 16),
        .destructive  : UIFont(name: buttonBold, size: 16)
    ]
    ...
    var buttonBgColorHighlighted: [DOAlertActionStyle : UIColor] = [
        .default : UIColor.brown.withAlphaComponent(0.8),
        .cancel  : UIColor.brown.withAlphaComponent(0.8),
        .destructive  : UIColor.cyan.withAlphaComponent(0.8)
    ]
    ...
    convenience init(title: String?, message: String?, preferredStyle: DOAlertControllerStyle) {
        ...
        //        let blurViewTopSpaceConstraint = NSLayoutConstraint(item: blurView, attribute: .Top, relatedBy: .Equal, toItem: self.view, attribute: .Top, multiplier: 1.0, constant: 0.0)
        //        let blurViewRightSpaceConstraint = NSLayoutConstraint(item: blurView, attribute: .Right, relatedBy: .Equal, toItem: self.view, attribute: .Right, multiplier: 1.0, constant: 0.0)
        //        let blurViewLeftSpaceConstraint = NSLayoutConstraint(item: blurView, attribute: .Left, relatedBy: .Equal, toItem: self.view, attribute: .Left, multiplier: 1.0, constant: 0.0)
        //        let blurViewBottomSpaceConstraint = NSLayoutConstraint(item: blurView, attribute: .Bottom, relatedBy: .Equal, toItem: self.view, attribute: .Bottom, multiplier: 1.0, constant: 0.0)
    ...
    func layoutView() {
        ...
        let hasTextView: Bool = textViews != nil && textViews!.count > 0
        ...
        if (hasTextField) {
            ...
            for (_, obj) in (textFields!).enumerated() {
                ...
                let padView = UIView(frame: CGRect(x: 0, y: 0, width: 3, height: textField.bounds.height))
        ...
        if (hasTextView) {
            ...
            textFieldContainerView.backgroundColor = textViewBgColor
            textFieldContainerView.layer.masksToBounds = true
            textFieldContainerView.layer.cornerRadius = textFieldCornerRadius
            textFieldContainerView.layer.borderWidth = 0.5
            textFieldContainerView.layer.borderColor = textFieldBorderColor.cgColor
            ...
            // TextFields
            for (_, obj) in (textViews!).enumerated() {
                let textView = obj as! UITextView
                textView.frame = CGRect(x: 0.0, y: textFieldContainerHeight, width: innerContentWidth, height: textView.frame.height)
                textFieldContainerHeight += textView.frame.height + 0.5
            }
    ...
    // Adds a text view to an alert.
    func addTextViewWithConfigurationHandler(_ configurationHandler: ((UITextView?) -> Void)!) {
        
        // You can add a text field only if the preferredStyle property is set to DOAlertControllerStyle.Alert.
        if (!isAlert()) {
            NSException.raise(NSExceptionName(rawValue: "NSInternalInconsistencyException"), format: "Text views can only be added to an alert controller of style DOAlertControllerStyleAlert", arguments:getVaList(["nil"]))
            return
        }
        if (textViews == nil) {
            textViews = []
        ...
        textView.frame.size = CGSize(width: innerContentWidth, height: textViewHeight)
...
extension DOAlertController: UITextViewDelegate {
